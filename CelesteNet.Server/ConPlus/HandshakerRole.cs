    ...
    public partial class HandshakerRole : NetPlusThreadRole {
        ...
        public async Task DoTCPUDPHandshake(Socket sock, CelesteNetTCPUDPConnection.Settings settings, TCPReceiverRole tcpReceiver, UDPReceiverRole udpReceiver, TCPUDPSenderRole sender) {
            ...
            try {
                ...
                string? playerUID = null, playerName = null,playerPhotoUrl = null, playerColor= null,playerPrefix = null;
                ...
                using (CancellationTokenSource tokenSrc = new()) {
                    ...
                    try {
                        ...
                        if (teapotRes != null)
                            (conFeatures, playerUID, playerName, clientOptions,playerPhotoUrl,playerColor,playerPrefix) = teapotRes.Value;
                    } catch {
                ...
                using (con.Utilize(out bool alive)) {
                    ...
                    if (!alive || !con.IsConnected)
                        return;
                    Server.CreateSession(con, playerUID, playerName, clientOptions,playerPhotoUrl,playerColor,playerPrefix);
                }
            } catch {
                con?.Dispose();
                sock.Dispose();
        ...
        // Also: I'm a Teapot
        private async Task<(IConnectionFeature[] conFeatures, string playerUID, string playerName, CelesteNetClientOptions clientOption,string playerPhotoUrl,string playerColor,string playerPrefix)?> TeapotHandshake<T>(Socket sock, uint conToken, T settings, string conUID) where T : new() {
...
{string.Format(Server.Settings.MessageTeapotVersionMismatch, teapotVer, CelesteNetUtils.LoadedVersion)}"
                ...
                string? errorReason = AuthenticatePlayerNameKey(playerNameKey, conUID, out string? playerUID, out string? playerName,out string? playerPhotoUrl,out string? playerColor,out string? playerPrefix);
                if (playerUID == null)
                    errorReason ??= "No UID";
                if (playerName == null)
                    errorReason ??= "No name";
...
Who wants some tea?"
                ...
                return (matchedFeats.Select(f => f.feature).ToArray(), playerUID, playerName, clientOptions,playerPhotoUrl,playerColor,playerPrefix);
            } finally {
        ...
        public string? AuthenticatePlayerNameKey(string nameKey, string conUID, out string? playerUID, out string? playerName, out string? playerPhotoUrl, out string? playerColor,out string? playerPrefix) {
            // New Auth Logic

            playerPrefix = playerUID = playerName = playerPhotoUrl = null;
            playerColor = null;
